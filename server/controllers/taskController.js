...
const createTask = asyncHandler(async (req, res) => {
  try {
    ...
    text =
      text +
      ` The task priority is set to: ${priority}. Please act accordingly. The task date is ${new Date(
        date
  ...
  try {
    ...
    text =
      ...
      ` The task priority is set to: ${
        task.priority
      } Please act accordingly. The task date is ${new Date(
        task.date
      ).toDateString()}. Thank you!`;
...
const updateTaskStage = asyncHandler(async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: true, message: "Task stage edited successfully!" });
  } catch (error) {
    return res.status(400).json({ status: false, message: error.message });
  }
});
...
const getTask = asyncHandler(async (req, res) => {
  try {
    ...
    console.log(error);
    throw new Error("Failed to fetch task. Please try again.", error);
  }
});
...
const trashTask = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      status: true,
      message: `Task trashed successfully! Check Trash for details.`,
    });
  } catch (error) {
    return res.status(400).json({ status: false, message: error.message });
  }
